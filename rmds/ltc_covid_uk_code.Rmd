...
Basic plot of the total number of deaths in each week
...
uk_deaths_data %>% 
  ...
  summarise(covid_deaths = sum(covid_deaths),
            pop = sum(pop)) %>% 
  mutate(uk_death_100k = covid_deaths/pop * 100000) %>% 
  my_datatable(.)
```
