...
```{r}
eng <- read_xlsx("data/ENGLAND_care_homes.xlsx", sheet = "HSCA Active Locations")
scot <- read_xlsx("data/SCOTLAND_care_homes.xlsx", sheet = "MDSF_data_31 May 2020")
ni <- read_xlsx("data/NI_care_homes.xlsx")
```
...
eng %<>% 
  ...
  select(`Location Name`, `Care homes beds`)  %>% 
...
```{r}
uk <- bind_rows(eng, scot, ni) %>% 
  mutate(country = factor(country,
                          levels = c("England", "Scotland", "Northern Ireland")))
```


```{r}
uk %>% 
  ggplot(aes(fct_rev(country), `Care homes beds`)) +
  geom_violin(alpha = 0.5, colour = "grey") +
  geom_boxplot(width = 0.2, aes(colour = country)) +
  scale_fill_wsj() +
